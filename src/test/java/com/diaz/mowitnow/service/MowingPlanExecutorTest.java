...
class MowingPlanExecutorTest {
    ...
    void should_execute_all_mower_programs_sequentially() {
        ...
        assertEquals(1, results.get(0).getPosition().x());
        assertEquals(3, results.get(0).getPosition().y());
        assertEquals(Orientation.N, results.get(0).getOrientation());

        assertEquals(5, results.get(1).getPosition().x());
        assertEquals(1, results.get(1).getPosition().y());
