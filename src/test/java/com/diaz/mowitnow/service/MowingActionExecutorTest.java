...
class MowingActionExecutorTest {
    ...
    private final MowingActionExecutor executor = new MowingActionExecutor(new Lawn(new Coordinates(5, 5)));
    ...
    void should_ignore_forward_action_when_next_position_is_outside_lawn() {
        Mower mower = new Mower(new Coordinates(5, 5), Orientation.N);

        executor.execute(mower, Action.A);

        assertEquals(5, mower.getCoordinates().x());
        assertEquals(5, mower.getCoordinates().y());
        assertEquals(Orientation.N, mower.getOrientation());
    }
    ...
    void should_execute_full_sequence_for_first_sample_mower() {
        ...
        assertEquals(1, result.getCoordinates().x());
        assertEquals(3, result.getCoordinates().y());
    ...
    void should_execute_full_sequence_for_second_sample_mower() {
        ...
        assertEquals(5, result.getCoordinates().x());
        assertEquals(1, result.getCoordinates().y());
